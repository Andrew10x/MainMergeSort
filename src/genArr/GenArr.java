...
public class GenArr {
    public static void main(String[] args) throws IOException {
        double[] arr = genArr(10_000_000, 20_000_000);
        writeInFile(arr);
        //readArr();
    }

    public static double[] genArr(int size, int maxRandNumber) {
        double[] arr = new double[size];

        for(int i=0; i<size; i++) {
            arr[i] = (Math.random()*maxRandNumber);
        }
        return arr;
    }

    public static void writeInFile(double[] arr)  {
        try(FileWriter writer = new FileWriter("D:\\genArr\\arr10_000_000.txt", false)){
